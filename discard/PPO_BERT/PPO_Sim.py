...
class PPO_SimModel(object):
    ...
    def runStep(self, action):
        ...
        if len(self.action)==0:
            ...
            ctrlData=alpha*self.action[0][1,:]+(1-alpha)*action[0,:]
            self.action.append(action)
        else:
            ctrlData=alpha*alpha*self.action[0][2,:]+(1-alpha)*alpha*self.action[1][1,:]+(1-alpha)*action[0,:]
            self.action.__delitem__(0)
            self.action.append(action)
        ...
        return copy.deepcopy(self.state), reward, done, copy.deepcopy(pos), copy.deepcopy(ctrlData)
