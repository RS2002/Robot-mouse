...
def Opt_with_points(ETG, ETG_T=0.4, points=None, b0=None, w0=None, precision=1e-4, lamb=0.5, plot=False, **kwargs):
    ...
    w_ = np.stack((x1, x2), axis=0).reshape(2, -1)
    b_ = np.array([b[0], b[1]])
    return w_, b_, points
...
def run_train_episode(agent, env, rpm, max_step, action_bound, w=None, b=None):
    ...
    if len(critic_loss_list) > 0:
        ...
        infos["critic_loss"] = "not train"
        infos["actor_loss"] = "not train"
    '''infos["success_rate"] = success_num / episode_steps'''
    infos['dis']=-info['pos'][1]
...
def main():
    ...
    if not args.eval:
        ...
        while total_steps < args.max_steps:
            ...
            logger.info('critic loss: {} actor loss: {}'.format(info['critic_loss'],info['actor_loss']))
            # Evaluate episode
            ...
            if args.ES and (total_steps + 1) // ES_EVERY_STEPS >= ES_test_flag and total_steps >= WARMUP_STEPS:
                while (total_steps + 1) // ES_EVERY_STEPS >= ES_test_flag:
                    ES_test_flag += 1
                    best_reward, avg_step, info = run_EStrain_episode(agent, env, rpm, e_step, act_bound, w, b)
                    ...
                    for ei in range(ES_TRAIN_STEPS):
                        ...
                        for solution in solutions:
                            ...
                            new_points = prior_points + points_add
                            w, b, _ = Opt_with_points(ETG=ETG_agent, ETG_T=args.ETG_T, w0=w0, b0=b0, points=new_points)
                            episode_reward, episode_step, info = run_EStrain_episode(agent, env, rpm, e_step, act_bound, w,
                                                                                     b)
                        ...
                        logger.info('ESSteps: {} Reward: {} step: {}  sigma:{}'.format(ES_step, np.max(fitness_list),
                                                                                       np.mean(steps), sig))
                        logger.info('average distance: {}  max distance: {}'.format(np.mean(dis),np.max(dis)))
